<?php
function aboss_logo_shortcode( $atts ) {
	...
	if( isset($aboss_opt['logo_main']['url']) && $aboss_opt['logo_main']['url']!=''){
		$html .= '<div class="logo">';

			if($atts['logo_link']=='yes'){
				$html .= '<a href="'.esc_url( home_url( '/' ) ).'" title="'.esc_attr( get_bloginfo( 'name', 'display' ) ).'" rel="home">';
			}
				$html .= '<img src="'.esc_url($aboss_opt['logo_main']['url']).'" alt="'.esc_attr( get_bloginfo( 'name', 'display' ) ).'" />';
...
function aboss_mainmenu_shortcode( $atts ) {
	$aboss_opt = get_option( 'aboss_opt' );
	...
	<div class="main-menu-wrapper">
		<div class="visible-small mobile-menu"> 
			<div class="mbmenu-toggler"><?php echo esc_html($aboss_opt['mobile_menu_label']);?><span class="mbmenu-icon"><i class="ion-android-menu"></i></span></div>
			<div class="clearfix"></div>
			<?php wp_nav_menu( array( 'theme_location' => 'mobilemenu', 'container_class' => 'mobile-menu-container', 'menu_class' => 'nav-menu' ) ); ?>
		</div> 
		<div class="<?php if(isset($aboss_opt['sticky_header']) && $aboss_opt['sticky_header']) {echo 'header-sticky';} ?> <?php if ( is_admin_bar_showing() ) {echo 'with-admin-bar';} ?>">
...
function aboss_roadcategoriesmenu_shortcode ( $atts ) {
	...
	if(isset($aboss_opt['categories_menu_home']) && $aboss_opt['categories_menu_home']) {
		$cat_menu_class .=' show_home';
	}
	if(isset($aboss_opt['categories_menu_sub']) && $aboss_opt['categories_menu_sub']) {
		$cat_menu_class .=' show_inner';
	}
	?>
	<div class="categories-menu visible-large <?php echo esc_attr($cat_menu_class); ?>">
		<div class="catemenu-toggler"><i class="ion-android-menu"></i><span><?php if(isset($aboss_opt)) { echo esc_html($aboss_opt['categories_menu_label']); } else { _e('Category', 'aboss'); } ?></span></div>
		<?php wp_nav_menu( array( 'theme_location' => 'categories', 'container_class' => 'categories-menu-container', 'menu_class' => 'categories-menu' ) ); ?>
		<div class="morelesscate">
			<span class="morecate"><i class="ion-ios-more-outline"></i><?php if ( isset($aboss_opt['categories_more_label']) && $aboss_opt['categories_more_label']!='' ) { echo esc_html($aboss_opt['categories_more_label']); } else { _e('More Categories', 'aboss'); } ?></span>
			<span class="lesscate"><i class="ion-ios-more-outline"></i><?php if ( isset($aboss_opt['categories_less_label']) && $aboss_opt['categories_less_label']!='' ) { echo esc_html($aboss_opt['categories_less_label']); } else { _e('Close Menu', 'aboss'); } ?></span>
		</div>
	</div>
	<?php
...
function aboss_roadlangswitch_shortcode( $atts ) {
	$aboss_opt = get_option( 'aboss_opt' );
	...
	if (class_exists('SitePress')) { ?>
		<div class="switcher">
			<div class="language"><label><?php echo esc_html_e('language:','aboss')?></label><?php do_action('icl_language_selector'); ?></div> 
			<div class="currency"><label><?php echo esc_html_e('currency:','aboss')?></label><?php do_action('currency_switcher'); ?></div> 
		</div> 
	<?php }
...
function aboss_roadsocialicons_shortcode( $atts ) {
	$aboss_opt = get_option( 'aboss_opt' );
	...
	if(isset($aboss_opt['social_icons'])) {
		echo '<ul class="social-icons">';
		foreach($aboss_opt['social_icons'] as $key=>$value ) {
			if($value!=''){
				if($key=='vimeo'){
...
function aboss_roadminicart_shortcode( $atts ) {
...
function aboss_roadproductssearch_shortcode( $atts ) {
	...
	if( class_exists('WC_Widget_Product_Categories') && class_exists('WC_Widget_Product_Search') ) { ?>
		<div class="header-search"> 
			<div class="search-categories-container">
				<div class="cate-toggler"><?php esc_html_e('All Categories', 'aboss');?></div>
...
function aboss_brands_shortcode( $atts ) {
	global $aboss_opt;
	$brand_index = 0;
	
	if(isset($aboss_opt['brand_logos'])) {
		$brandfound = count($aboss_opt['brand_logos']);
	}
	$atts = shortcode_atts( array(
							'rowsnumber' => '1',
							'colsnumber' => '6',
	...
	if(isset($aboss_opt['brand_logos']) && $aboss_opt['brand_logos']) {
		$html .= '<div class="brands-carousel" data-col="'.$atts['colsnumber'].'">';
			foreach($aboss_opt['brand_logos'] as $brand) {
				if(is_ssl()){
...
function aboss_counter_shortcode( $atts ) {
...
function aboss_list_categories_shortcode( $atts ) {
...
function aboss_roadtimesale_shortcode( $atts ) {  
	$aboss_opt = get_option( 'aboss_opt' );
	...
	ob_start();
	if(isset($aboss_opt['sale-date-time'])) {
		echo '<div class="countdownsale" data-date="'.$aboss_opt['sale-date-time'].' 00:00:00" style="width: 470px; height: 117px;"></div>';
	}
	$html .= ob_get_contents();
...
function aboss_categoriescarousel_shortcode( $atts ) {
	global $aboss_opt;
	$categories_index = 0;
	if(isset($aboss_opt['cate_images'])){
		$categoriesfound = count($aboss_opt['cate_images']);
	}
	...
	if(isset($aboss_opt['cate_images'])){
		$html .= '<div class="categories-carousel" data-col="'.$atts['colsnumber'].'">';
			foreach($aboss_opt['cate_images'] as $categories) {
				if(is_ssl()){
...
function aboss_latestposts_shortcode( $atts ) {
	global $aboss_opt;
	$post_index = 0;
	$atts = shortcode_atts( array(
		'posts_per_page' => 5,
	...
	if($atts['image']=='wide'){
		$imagesize = 'aboss-post-thumbwide';
	} else {
		$imagesize = 'aboss-post-thumb';
	}
	$html = '';
			...
			foreach ( $postslist as $post ) {
				...
				$html.='<div class="item-col">';
					...
					$html.='<div class="post-thumb">'; 
						...
						$html.='<a class="readmore" href="'.get_the_permalink($post->ID).'">'.'<span>' .esc_html($aboss_opt['readmore_text']). '</span>'.'</a>';
					$html.='</div>';
					
					$html.='<div class="post-info">';    
						$html.='<h3 class="post-title"><a href="'.get_the_permalink($post->ID).'">'.get_the_title($post->ID).'</a></h3>';	  
						$html.='<span class="post-author">';
							$html.= sprintf( wp_kses(__( '%s', 'aboss' ), array('a'=>array('href'=>array()))),'By <a href="'.$author_url.'">'.$author_name.'</a>' );
						$html.='</span>'; 
						$html.='<div class="post-excerpt">';
							$html.= Aboss_Class::aboss_excerpt_by_id($post, $length = $atts['length']);
						$html.='</div>'; 
...
function aboss_contact_map( $atts ) {
	global $aboss_mapid;
	
	if(!isset($aboss_mapid)){
		$aboss_mapid = 1;
	} else {
		$aboss_mapid++;
	}
	...
	$html.='<div class="map-wrapper">';
		$html.='<div id="map'.$aboss_mapid.'" class="map" style="height: '.$map_height.'px"></div>';
	$html.='</div>';
	...
	<script type="text/javascript">
		jQuery(document).ready(function(){
			jQuery('#map<?php echo esc_attr($aboss_mapid);?>').gMap({
				scrollwheel: false,
...
function aboss_magnifier_options($att) {
